...
methods(Static)
    ...
    function msgData = readCommandTrajectory(msgFile)
        ...
        msgData.q = [cellfun(@(m) double(m.Q.X), msgFile), ...
                    cellfun(@(m) double(m.Q.Y), msgFile), ...
                    cellfun(@(m) double(m.Q.Z), msgFile)];
    ...
    function msgData = sensorsmsgsImu(msgFile)
        tSecs = cellfun(@(m) double(m.Header.Stamp.Sec), msgFile);
        tNsecs = cellfun(@(m) double(m.Header.Stamp.Nsec), msgFile);

        msgData.t = tSecs + tNsecs*1e-9;
        
        msgData.accel = [cellfun(@(m) double(m.LinearAcceleration.X), msgFile), ...
                            cellfun(@(m) double(m.LinearAcceleration.Y), msgFile), ...
                            cellfun(@(m) double(m.LinearAcceleration.Z), msgFile)];
        msgData.gyro = [cellfun(@(m) double(m.AngularVelocity.X), msgFile), ...
                            cellfun(@(m) double(m.AngularVelocity.Y), msgFile), ...
                            cellfun(@(m) double(m.AngularVelocity.Z), msgFile)];
        msgData.quat = [cellfun(@(m) double(m.Orientation.W), msgFile), ...
                            cellfun(@(m) double(m.Orientation.X), msgFile), ...
                            cellfun(@(m) double(m.Orientation.Y), msgFile), ...
                            cellfun(@(m) double(m.Orientation.Z), msgFile)];
    end
